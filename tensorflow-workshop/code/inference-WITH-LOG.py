...
def input_handler(data, context):
    ...
    if context.request_content_type == 'application/x-image':
        ...
        logging.info('opening bytes as Pillow Image')
        stream = io.BytesIO(data.read())
        img = Image.open(stream)
        logging.info('resizing to {}, {}'.format(WIDTH, HEIGHT))
        img = img.resize((WIDTH, HEIGHT))
        x = np.asarray(img)
        curr_shape = x.shape
        new_shape = (1,) + curr_shape
        logging.info('reshaping to: {} from {}'.format(new_shape, curr_shape))
        x = x.reshape(new_shape)
        logging.info('completed reshape')
        instance = preprocess_input(x)
        final_input = instance.tolist()
        logging.info('final input:\n{}'.format(final_input))
